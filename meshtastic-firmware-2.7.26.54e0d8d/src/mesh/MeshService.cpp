...
// Extern variabili ISR anemometro
extern volatile uint32_t wind_pulse_count;
extern volatile uint32_t wind_min_interval_us;
...
void MeshService::fanControlTask(void *pvParameters) {
    ...
    for (;;) {
       ...
       aggiornaMeteoLocale(cicli); // <--- Passiamo il contatore dei cicli qui!
        ...
        // =========================================================================
        // =========================================================================
        // OGNI 30 SECONDI: Controllo Ventola e Reboot
        // OGNI 60 SECONDI: Invio Telemetria MQTT

        if (cicli % 6 == 0) { // Questo avviene ogni 30 secondi (ciclo 0 e ciclo 3)
            checkInternalFan();
            checkAutoReboot();
        }
